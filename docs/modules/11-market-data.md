...
### 3.17 Vendor specifics: Bloomberg, Refinitiv, ICE

The market-data category has three vendors that dominate the warehouse's daily intake at most firms. Each has its own delivery shape, its own identifier conventions, and its own firm-side gotchas; the discipline of §3.9's vendor → cleansed → official pipeline absorbs them in a unified way, but the per-vendor specifics are worth naming explicitly because they shape the integration patterns the data team will inherit.

**Bloomberg.** The most widely deployed market-data provider in the field, best known for the Bloomberg Terminal but in the warehouse context primarily for the BPIPE feed (real-time and snapshot market data over a binary protocol), the B-PIPE Data License (a file-and-API delivery alternative for firms that prefer batch shapes), and the BVAL composite (Bloomberg's evaluated end-of-day pricing for fixed income). Identifier conventions: Bloomberg's tickers (`AAPL US Equity`, `USDJPY Curncy`) are the daily working surface, with FIGI codes (administered by Bloomberg under the Open Symbology initiative) as the open-standard alternative. Firm-side gotchas: the BPIPE binary protocol requires a dedicated parser library and an in-network Bloomberg connection, which is a substantial integration commitment; licensing constraints are strict (most BPIPE contracts forbid downstream redistribution and limit the consumer count, which the warehouse must enforce through access controls and per-consumer tagging); BVAL pricing has a known lag relative to the trading day's true close because the evaluation methodology requires post-trade observation, so the warehouse cannot treat BVAL as a real-time feed even though Bloomberg ships it through the same channel as the real-time prices. The vendor → cleansed → official pipeline of §3.9 absorbs Bloomberg as a vendor input and applies the firm's cleansing on top.

**Refinitiv (LSEG).** The second of the two historical dominants in the market-data space, formerly Reuters, now part of the London Stock Exchange Group. Best known in the warehouse context for the Eikon Data API and the legacy RFA/RDP feed (the modern RDP is the LSEG Data Platform, a cloud-native API; older RFA deployments persist at firms that have not yet migrated). Refinitiv ships reference data, prices, curves, and a substantial news-and-fundamentals product that the warehouse may or may not consume depending on which downstream functions need it. Identifier conventions: Refinitiv's RIC (Reuters Instrument Code) is the daily working surface (`AAPL.O`, `USD=`), with the firm typically maintaining a RIC-to-firm-canonical xref alongside the Bloomberg ticker xref. Firm-side gotchas: Refinitiv's curve and surface conventions sometimes disagree with Bloomberg's (the daycount used to bootstrap a particular currency's curve, the strike convention on a particular vol surface) and the warehouse must catch these in cross-vendor reconciliation; the Eikon-vs-RDP migration is ongoing at most firms and the integration patterns differ between the two; licensing constraints are similar to Bloomberg's in spirit but different in detail, and the warehouse must enforce them per-product.

**ICE Data Services.** Intercontinental Exchange's data products, best known for fixed-income evaluations (ICE Bond Pricing, the ICE end-of-day curves), credit indices (the iTraxx and CDX series and their constituent prices), and the broader ICE end-of-day price portfolio. ICE typically ships through file delivery (sFTP drops to a shared landing zone) for the EOD products, and through an API for ad-hoc lookups; the volumes are moderate, the latency is end-of-day, and the integration pattern is the file-drop pattern from §3.9. Identifier conventions: ICE typically uses ISINs and CUSIPs as the primary instrument identifiers for fixed-income, with proprietary ICE codes for the index constituents. Firm-side gotchas: the timing of ICE's official evaluations is later in the evening than some firms' EOD cuts (the warehouse must align the snapshot timing or accept a one-day lag on the ICE-sourced rows); the CDX/iTraxx index roll dates can introduce identifier discontinuities that the warehouse must catch via DQ checks; the licensing for ICE's higher-frequency products is structured per product and per consumer in a way the warehouse must track.

A closing pointer. The three vendors above are the dominant *market-data* category. For trading-platform vendors (Murex, Calypso, Polypaths, Quantserver, Summit, Front Arena, Sophis, and the in-house pricing engines that fit the same shape) the warehouse needs a different integration pattern — the trade-and-position-and-sensitivity entities require silver-layer conformance against the firm's master data in a way the curves-and-surfaces of M11 do not. [Module 23](23-vendor-systems-framework.md) is the framework that systematises that broader vendor-onboarding work, and [Module 24](24-murex-applied.md) is the first applied case study. The framework treats market-data vendors and trading-platform vendors as siblings under a shared discipline, with this module remaining the home for the market-data half.
